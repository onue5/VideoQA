...
class OurVideoQAModel(nn.Module):
    ...
    def forward(self, config, p, p_mask, p_lens, s_lens, q, q_mask, q_lens, span_masks, dev):
        ...
        :param span_masks: (batch_size, p_len, p_len)
        :return:
        """
        ...
        # scores = scores * span_masks
        # scores = scores.view(batch_size, max_p_len * max_p_len)  # (batch_size, max_p_len, max_p_len)

        scores = self.softmax(scores)

        scores = scores.view(batch_size, max_p_len, max_p_len)
        scores = scores * span_masks

        # if dev:
        #     nonzero_indexes = scores[0].nonzero()
