...
import numpy as np
from codes.rasorsent_hint.model import OurVideoQAModel
from codes.rasorsent_hint.reader import Dataset
from codes.utils import load_glove_vectors, measure_diff, encode_answer_index
...
def _eval_dev(model, dev_dataset, verbose=False):
    ...
    while dev_dataset.is_next_batch_available():
        batch = dev_dataset.next_batch(config.batch_size, train=False)
        _, p, p_mask, p_len, s_lens, q, q_mask, q_len, a, span_mask = batch
        ...
        scores = model(
            ...
            Variable(q_len, requires_grad=False), Variable(span_mask, requires_grad=False), dev=True
        )
...
def _train(model, train_dataset, epochid):
    ...
    while train_dataset.is_next_batch_available():
        batch = train_dataset.next_batch(config.batch_size, train=True)
        _, p, p_mask, p_len, s_lens, q, q_mask, q_len, a, span_mask = batch
        ...
        scores = model(
            ...
            Variable(p_len, requires_grad=False), Variable(s_lens, requires_grad=False),
            Variable(q, requires_grad=False), Variable(q_mask, requires_grad=False),
            Variable(q_len, requires_grad=False), Variable(span_mask, requires_grad=False), dev=False
        )
